...
import { useGetDonationHistoryById } from "../hooks/useGetDonationHistoryById";
import { IMAGE_URL } from "../../../../constants/constants";
import { useCreateRating } from "../../../recipient/RequestFood/hooks/useCreateRating";

export default function DonorRatingPage() {
  const navigate = useNavigate();
  const { foodId } = useParams();
  const { foods: post, loading, fetchDonationHistoryById } = useGetDonationHistoryById();
  const { createRating,
  ...
  useEffect(() => {
    fetchDonationHistoryById(foodId);
  }, [foodId]);

  const ratingData = {
    name: post[0]?.title || "Food Item",
    description: post[0]?.description || "No description available.",
    image:
      post[0]?.photo
        ? IMAGE_URL +  post[0]?.photo
  ...
  const handleSubmit = (e) => {
    ...
    createRating({
      "receiverId": post[0]?.acceptedRequest?.receiver?._id,
      "rating": rating,
      "comment": review,
    });
    // Simulate submission
